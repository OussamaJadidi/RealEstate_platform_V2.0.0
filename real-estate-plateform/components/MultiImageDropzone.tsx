...
const MultiImageDropzone = React.forwardRef<HTMLInputElement, InputProps>(
  (
    ...
    return (
      <div>
        <div className="grid grid-cols-[repeat(1,1fr)] gap-2 sm:grid-cols-[repeat(2,1fr)] 2xl:grid-cols-[repeat(3,1fr)] ">
          {/* Images */}
          ...
          {(!value || value.length < (dropzoneOptions?.maxFiles ?? 0)) && (
            <div
              {...getRootProps({
                className: dropZoneClassName + " aspect-video ",
              })}
            >
              {/* Main File Input */}
              <input ref={ref} {...getInputProps()} autoFocus={false} />
